...
func NewStorage(conf StorageConfig) (*Storage, error) {
	...
	return &Storage{svc}, nil
}

// Get copies an object from GS to the host path.
func (gs *Storage) Get(ctx context.Context, rawurl string, hostPath string, class tes.FileType) error {
...
func (gs *Storage) Put(ctx context.Context, rawurl string, hostPath string, class tes.FileType) ([]*tes.OutputFileLog, error) {
...
func (gs *Storage) put(ctx context.Context, rawurl, hostPath string) error {
...
func (gs *Storage) Supports(rawurl string, hostPath string, class tes.FileType) bool {
	_, err := parse(rawurl)
	if err != nil {
		return false
	}
